...
const ProjectCard = ({
    ...
    const footerOverlay =
        darkText ? "before:bg-white/10 border-black/20" : "before:bg-white/10 border-white/20";
            ...
            <Tilt options={{ max: 45, scale: 1, speed: 450 }} className="w-full">
                ...
                >
                    <Image
                        ...
                        src={image}
                        removeWrapper
                        className="hidden sm:block object-cover w-full h-[330px] rounded-2xl"
                    />
                    ...
                    >
                        <div className="flex min-w-0 flex-col pr-3">
                            ...
                            <p className={`${nameCls} font-semibold truncate text-lg sm:text-l`}>
                                {name}
                            </p>
                            {/* description */}
                            <p className={`${descCls} line-clamp-2 text-sm sm:text-base leading-relaxed`}>
                                {description}
                            </p>
                        </div>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                    {tags.map((tag) => (
                        <p key={`${name}-${tag.name}`} className={`text-[15px] sm:text-[16px] ${tag.color}`}>
                            #{tag.name}
                        </p>
                    ))}
                </div>
  ...
  return (
      <>
        ...
        <div className="w-full flex">
          ...
          >
              Portofoliul de mai jos arată experiența și abilitățile echipei Generation Salez în realizarea de soluții digitale orientate spre rezultate. Fiecare exemplu include o descriere scurtă și stack-ul tehnic, evidențiind modul în care gestionăm provocări complexe, combinăm tehnologii variate și ducem proiectele la bun sfârșit.
          </motion.p>
        </div>
